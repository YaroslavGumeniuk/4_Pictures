...
window.addEventListener("DOMContentLoaded", function () {
  ...
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.main-slider', '.slider-wrapper', '.main-slider-item', '', {
    isCycling: true,
    direction: "down",
    interval: 2000
  });
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.feedback-slider', '.slider-wrapper', '.feedback-slider-item', '.main-slider-btn', {
    isCycling: true,
    direction: "right",
    interval: 2000
  });
...
var acordion = function acordion(triggersSelector, itemsSelector) {
  ...
  txtInputs.forEach(function (input) {
    ...
    input.addEventListener('input', function (e) {
      if (input.value.match(/[^а-яё 0-9]/ig)) {
        input.value = "Введите на русском";
        e.preventDefault();
      }
    });
  });
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      if (input.getAttribute('data-upload')) {
        ...
        input.previousElementSibling.textContent = "\u0424\u0430\u0439\u043B ".concat(name, " \u043E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D");
      }
    });
  });
};
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener("click", function (e) {
        ...
        giftImg.style.marginRight = "".concat(scroll, "px");
  ...
  function openByScroll(selector) {
    window.addEventListener("scroll", function () {
      ...
      if (!btnPresset && window.pageYOffset + document.documentElement.clientHeight >= scrollHeight) {
...
var sliders = function sliders(slider, wrapper, items, controls, config) {
  var mainElement = document.querySelector(slider),
      // основный элемент блока
  sliderWrapper = mainElement.querySelector(wrapper),
      // обертка для .slider-item
  sliderItems = mainElement.querySelectorAll(items),
  ...
  var getPositions = {
    getIndexMin: function getIndexMin() {
      var itemIndex = 0;
    ...
    getIndexMax: function getIndexMax() {
      var itemIndex = 0;
      itemsArray.forEach(function (item, index) {
        if (item.positionSlide > itemsArray[itemIndex].positionSlide) {
          itemIndex = index;
  ...
  var cycle = function cycle(direction) {
  ...
  var controlClick = function controlClick(e) {
    var direction = e.target.parentNode.classList.contains("main-next-btn") ? "right" : "left";
    transformItem(direction);
    clearInterval(interval); // cycle(configSlider.direction); // при клике на кнопку цикл продолжается
  };

  var setUpListeners = function setUpListeners() {
    if (controls) {
      var sliderControls = mainElement.querySelectorAll(controls);
